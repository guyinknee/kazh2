...
const MapManager = {
    ...
    addLayer(layerConfig) {
        ...
        if (layer) {
            ...
            if (layerConfig.defaultVisible) {
                ...
                // Ensure key point layers sit on top
                try { if (layerConfig.id === 'renewable-points' && layer.bringToFront) layer.bringToFront(); } catch(e) {}
                try { if (layerConfig.id === 'power-stations' && layer.bringToFront) layer.bringToFront(); } catch(e) {}
    ...
    createMarkerLayer(config) {
        const rawData = AppState.data[config.dataSource];
        const data = this.normalizeDataToArray(rawData, 'Point');
        console.log(`Creating marker layer for ${config.id}:`, rawData);
    ...
    createMarker(point, config) {
        try {
            ...
            // Special handling for Renewable Energy markers: use emoji icons
            else if (config.dataSource === 'renewablePoints') {
            // Prefer res_type (your geojson has it), fallback to subtype/type text
            const raw = (
                point.res_type ||
                point.subtype ||
                point.type ||
            ...
            const t =
                raw.includes('wind') || raw.includes('вэс') ? 'wind' :
                raw.includes('solar') || raw.includes('сэс') ? 'solar' :
                raw.includes('hydro') || raw.includes('гэс') ? 'hydro' :
                'solar'; // fallback (pick whatever you prefer)

            // Cache icons so you don’t create hundreds of L.icon objects
            this._renewableIcons = this._renewableIcons || {};
            if (!this._renewableIcons[t]) {
                const cfg = LayersConfig.markerIcons.renewable[t]; // from layersConfig.js :contentReference[oaicite:3]{index=3}
                this._renewableIcons[t] = L.icon({
                ...cfg,
                // Make them smaller for performance/visual density
                iconSize: [22, 22],
                iconAnchor: [11, 11],
                popupAnchor: [0, -11],
                });
            ...
            icon = this._renewableIcons[t];
            }

            // Special handling for Major Thermal Power Stations
            else if (config.dataSource === 'powerStations') {
                icon = L.divIcon({
    ...
    getMarkerIcon(point, config) {
        // Emoji-based icons for hydrogen-relevant energy layers (no external assets needed)
        try {
            if (config?.dataSource === 'renewablePoints') {
                const rawType = (point.type || point.tech || point.technology || point.kind || point.source || '').toString().toLowerCase();
                const t = rawType.includes('wind') || rawType.includes('вэс') ? 'wind'
                        : rawType.includes('solar') || rawType.includes('сэс') ? 'solar'
                        : rawType.includes('hydro') || rawType.includes('гэс') ? 'hydro'
                        : 'renewable';
                const emoji = (t === 'wind') ? '🌬️' : (t === 'solar') ? '☀️' : (t === 'hydro') ? '💧' : '♻️';
                return L.divIcon({
                    className: 'emoji-marker renewable-emoji',
            ...
            if (config?.dataSource === 'powerStations') {
                const fuelRaw = (point.fuel || point.fuel_type || point.fueltype || point.primary_fuel || '').toString().toLowerCase();
                const isGas = fuelRaw.includes('gas') || fuelRaw.includes('газ');
                const emoji = isGas ? '🔥🟦' : '🔥⬛'; // gas vs coal(default)
                return L.divIcon({
                    className: 'emoji-marker thermal-emoji',
            ...
            console.warn('Emoji icon fallback failed:', e);
        }
   ...
   // Normalize data sources that might be provided either as arrays or as GeoJSON FeatureCollections
normalizeDataToArray(data, geometryType) {
  if (!data) return [];
    ...
    if (geometryType === 'Point') {
      ...
      data.features.forEach(f => {
        ...
        if (g.type === 'Point' && Array.isArray(g.coordinates)) {
          ...
          out.push({ ...props, latitude: lat, longitude: lon });
        }
...
createPolylineLayer(config) {
        const rawData = AppState.data[config.dataSource];
        const data = this.normalizeDataToArray(rawData, 'LineString');
...
selectFeature(e, feature, config) {
  ...
  const isProgrammatic = !e.latlng; // dropdown calls selectFeature without latlng

  // Clear previous selection (use previous config, not the new one)
  if (this.selectedFeature) {
    const prevConfig = this.selectedFeatureConfig;
    const prevStyle = prevConfig?.style || LayersConfig.styles[prevConfig?.id] || prevConfig?.defaultStyle;
    this.selectedFeature.setStyle(prevStyle || LayersConfig.styles.regions); // safe fallback
  }

  // Set new selection
  this.selectedFeature = e.target;
  this.selectedFeatureConfig = config;
  this.selectedFeature.setStyle(LayersConfig.styles.selected);

  // ✅ On MAP click: show popup first at click point
  // (sidebar/programmatic selection should NOT open popup / fitbounds here)
  if (!isProgrammatic) {
  ...
  if (config.id === 'regions' && feature?.properties) {
    ...
    // store FULL feature (same as sidebar path expects)
    AppState.selectedRegion = feature;

    // update dropdown value WITHOUT firing change
    const regionSelect = document.getElementById('region-select');
    if (regionSelect && regionSelect.value !== regionName) {
      regionSelect.value = regionName;
    }

    // show schematics immediately (no recursion)
    try {
      const p = feature.properties || {};
      const regionLabel =
        p.region_name_en || p.name_en || p.region_name || p.name || p.NAME_1 || 'Region';
      ...
      const waterType =
        ({ freshwater: 'fresh', brackish: 'brackish', treated: 'waste', groundwater: 'ground' }[waterRaw]) || 'fresh';
      ...
      // ✅ After schematics opens, reposition popup so it avoids the panel
      if (!isProgrammatic) {
        setTimeout(() => {
          PopupManager.reflow();
        }, 50);
      ...
      console.warn('Schematics update failed:', err);
    }
  }
},
